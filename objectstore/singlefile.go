...
func CreateSingleFileBackup(volume *Volume, snapshot *Snapshot, filePath, destURL, endpoint string,accesskey string, secretkey string) (string, error) {
	driver, err := GetObjectStoreDriver(destURL, endpoint,accesskey, secretkey)
	if err != nil {
		return "", err
	}
...
func RestoreSingleFileBackup(backupURL, endpoint,accesskey string, secretkey string, path string) (string, error) {
	driver, err := GetObjectStoreDriver(backupURL, endpoint,accesskey, secretkey)
	if err != nil {
		return "", err
	}
...
func DeleteSingleFileBackup(backupURL, endpoint string,accesskey string, secretkey string) error {
	driver, err := GetObjectStoreDriver(backupURL, endpoint, accesskey, secretkey)
	if err != nil {
		return err
	}
